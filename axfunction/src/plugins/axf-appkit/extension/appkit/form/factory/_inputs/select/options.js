...
select.options = function( options ) {
  ...
  let value = lib.form.collection.value( options.value )
  let collection = lib.form.collection.from( options.collection, { value: value } )
  ...
  collection = collection.map( function( option ) {
    if ( option.value && lib.form.collection.includes( value, option.value ) ) {
