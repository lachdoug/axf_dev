...
AxFrameworkExtensionsFormBuilderCustomFields.prototype.codemirror = function(
	name,
	options
) {
...
// debugger
							...
							_setMode: function ( mode ) {
								// debugger
								var bestMode = mode === "" ? mode : ( mode || this._selectedMode || options.mode || "" );
								this._selectedMode = bestMode;
								// debugger;
								if ( this._codemirror ) {
									this._codemirror.setOption( "mode", bestMode );
									this.
										closest("codemirror").
		...
		{
			_setMode: function( mode ) {
				this.querySelector("textarea")._setMode( mode );
			},

			_labelOnclick: function (e) {
				if ( e.target.closest("label") === this ) {
					this.querySelector("textarea")._codemirror.focus();
					return false;
				} else {
					return true;
				};
