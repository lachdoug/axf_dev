ax.extensions.form.factory.checkboxes = function(
  ...
  let checkboxesComponents = options.collection.map( function( checkbox, i ) {
    ...
    let label = a.label( checkbox[1], { onclick: function () { this.previousSibling.click() } } )

    return a.checkbox( [ check, label ] )

  } );

  let checkboxesTag = Object.assign(
    {
      $value: function() {
  ...
  return a.checkboxes( checkboxesComponents, checkboxesTag );
