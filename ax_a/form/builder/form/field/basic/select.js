...
AxFunctionExtensionsFormBuilder.prototype.select = function(
  name,
  options = {}
) {
  ...
  if ( attributes && attributes.multiple ) {
    name = this.lib.fieldNameForCollection( name );
  };
  ...
  value = this.lib.fieldValueForCollection( options.value );
  collection = collection.map( function( option ) {
    // debugger;
    if ( x.lib.arrayIncludes( value, option.value ) ) {
